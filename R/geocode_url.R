...
#'   \item \strong{address:} character; the user supplied physical address used for geocoding (prior to Google's formatting).
#'   }
...
geocode_url <- function(address, auth="standard_api", privkey=NULL,
	...
	coord <- t(vapply(json,function(x) {
		if(x$status=="OK") {
			lat <- as.character(x$results$geometry$location$lat)
			lng <- as.character(x$results$geometry$location$lng)
			location_type <- as.character(x$results$geometry$location_type)
			formatted_address <- as.character(x$results$formatted_address)
			...
			return(c(NA, NA, NA, NA, x$status, x$error_message))
		}
	}, character(6)))
	...
	if(verbose) cat("NOTE: There was", nrow(out[out$status=="INVALID_SIGNATURE", ]), "authentication failure(s) in the returned data.")
